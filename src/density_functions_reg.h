...
template<typename TDOUBLE>
bool loadGammaParams(GAMMA2_REG<TDOUBLE> &gamma1, GAMMA2_REG<TDOUBLE> &gamma2, AppOptions &options)
{
    ...
    while (std::getline(ifs, lineBuffer))
    {
        ...
        if (!ss.str().empty())
        {
            ...
            if (value1 == "gamma1.b0")
            {
                ...
                {
                    std::cerr << "ERROR: could not read second value for gamma1.b0\n";
                    return false;
                }
            ...
            else if (value1 == "gamma1.b1")
            {
                if (!std::getline(ss, value2, '\t')) 
                {
                    std::cerr << "ERROR: could not read second value for gamma1.b1\n";
                    return false;
                }
            ...
            else if (value1 == "gamma2.b0")
            {
                if (!std::getline(ss, value2, '\t')) 
                {
                    std::cerr << "ERROR: could not read second value for gamma2.b0\n";
                    return false;
                }
            ...
            else if (value1 == "gamma2.b1")
            {
                if (!std::getline(ss, value2, '\t')) 
                {
                    std::cerr << "ERROR: could not read second value for gamma2.b1\n";
                    return false;
                }
